...
load("/Users/arahjou/Desktop/BodyTime_Final_Version/06_FinalDataset_2024/ht1.Rda")
...
```{r}
rm(list = ls(all = TRUE)) 
load("/Users/arahjou/Desktop/BodyTime_Final_Version/06_FinalDataset_2024/ht2.Rda")
...
m3 <- lm(hair_time ~  sex + ns(age, df= 2), data = ht2)
m4 <- gam(hair_time ~  sex + s(age), data = ht2)
jtools::summ(m1, digits = 3, confint = T)
print(rep("******************", 10))
jtools::summ(m2, digits = 3, confint = T)
print(rep("******************", 10))
jtools::summ(m3, digits = 3, confint = T)
print(rep("******************", 10))
summary(m4, digits = 3, confint = T)
```
...
#### BIC: linear vs. spline model

```{r}
print(paste0("BIC for spline model: ", BIC(m1)))
print(paste0("BIC for linear model: ", BIC(m2)))
print(paste0("BIC for linear model: ", BIC(m3)))
print(paste0("BIC for linear model: ", BIC(m4)))
```
...
mod_work <- ggplot() +
  ...
  scale_y_continuous("Predicted DLMO (hours)",
                     ...
                     labels = c("20.00", "20.50", "20.75", "21.00", "21.25", "21.50"),
                     limits = c(-3.8, -2.7)) +
  theme(plot.background = element_rect(fill = "transparent", colour = "transparent"),
        plot.margin = margin(.8, .8, .8, .6, "cm"),
        panel.border = element_rect(color = "black", fill = NA, linewidth = 1),
...
load("/Users/arahjou/Desktop/BodyTime_Final_Version/06_FinalDataset_2024/ht4.Rda")
...
# Fit models
m1 <- lm(hair_time ~ sex + bs(age) + bs(workdays), data = ht2)  # spline for workdays
m2 <- lm(hair_time ~ sex + bs(age) + workdays, data = ht2)      # linear for workdays

# Print BIC values
print(paste0("BIC for spline model: ", BIC(m1)))
print(paste0("BIC for linear model: ", BIC(m2)))

# Predicted effects for workdays (spline model)
eff_wd_spline <- ggeffects::predict_response(m1, margin = "marginalmeans", terms = "workdays")
eff_wd_spline <- eff_wd_spline %>% rename(hair_time = predicted)
...
means <- ht2 %>%
  ...
  summarise(
    mean_age = mean(age),
    se_hair_time = sd(hair_time) / sqrt(n()),
    mean_hair_time = mean(hair_time),
    n = n()
  )
means$x <- as.numeric(as.character(means$workdays))
...
# Corrected hair_time
wd_age <- ggeffects::predict_response(m1, margin = "marginalmeans", terms = "age[18:70]")
wd_all <- ggeffects::predict_response(m1, margin = "marginalmeans", terms = c("age[18:70]", "sex"))

# Convert wd_all safely and extract needed columns
corr_wd <- as_tibble(wd_all) %>%
  dplyr::select(x, group, predicted) %>%
  rename(age = x, sex = group, predicted_hair_time = predicted)
...
means2 <- db %>%
  ...
  summarise(
    mean_age = mean(age),
    se_hair_time = sd(hair_time_corrected) / sqrt(n()),
    mean_hair_time = mean(hair_time_corrected),
    n = n()
  )
means2$x <- as.numeric(as.character(means2$workdays))

# Plot with spline prediction
workdays_fig_spline <- ggplot() +
  geom_point(data = means, aes(x = x, y = mean_hair_time),
             color = "#a3bbbf", alpha = 0.5) +
  geom_errorbar(data = means, aes(x = x, ymin = mean_hair_time - se_hair_time, ymax = mean_hair_time + se_hair_time),
                linewidth = 0.3, width = 0.1, color = "#a3bbbf", alpha = 0.5) +
  ...
  geom_text(data = means2, aes(label = n, x = workdays + 0.25, y = mean_hair_time + 0.35),
            color = "#ff4d00") +
  geom_line(data = eff_wd_spline, aes(x = x, y = hair_time), color = "#0072B2") +
  geom_ribbon(data = eff_wd_spline, aes(x = x, ymin = conf.low, ymax = conf.high), alpha = 0.2) +
  theme_bw() +
  scale_x_continuous("Workdays", breaks = seq(0, 7, 1)) +
  scale_y_continuous("Predicted DLMO (hours)",
                     breaks = c(-4, -3.5, -3, -2.5, -2, -1.5, -1, -0.5, 0),
